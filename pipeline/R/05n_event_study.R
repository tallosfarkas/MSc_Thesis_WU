...
write_json(list(window_def=windows, win=WIN, est_window=c(-(EST_LEN+EST_GAP-1L),-EST_GAP),
           ...
           vS(file.path(ANA,"event_study.json")), pretty=TRUE, auto_unbox=TRUE, na="null", digits=8)
say("  wrote out/analysis/event_study.json")

# ---- 8. plot CAR paths (market model; GeoExposure + dCC, Q1 vs Q5) -----------
ok <- tryCatch({
  png(file.path(ANA,"event_study_car.png"), width=1100, height=520, res=110)
  par(mfrow=c(1,2), mar=c(4,4,3,1))
  for (m in c("GeoExposure","dCC")){ a <- res[[m]]$car_mm
