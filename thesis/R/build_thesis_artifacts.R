...
local({
  ...
  lab <- c(GeoExposure="GeoExposure", dCC="$\\Delta$GeoExposure", GeoRisk="GeoRisk", GeoSentiment="GeoSentiment")
...
local({
  ...
  lrows <- vapply(names(ld), function(k)
    sprintf("%s & %s & %s \\\\", k, fnum(ld[[k]]$beta, 4), cellpt(0, ld[[k]]$t)), character(1))
  writetab("tab_spanning_loadings.tex", tabular("lcc",
    c("Factor & $\\beta$ & $(t)$\\\\"), lrows)) })

## gvkey (firm-level) robustness -- GeoRisk
local({
  gv <- J("crsp_gvkey.json")
  rows <- c(sprintf("Monthly (firm) & %s~%s \\\\",
              cellp(gv$monthly$q3$ew_alpha[gv$monthly$q3$measure=="GeoRisk"], gv$monthly$q3$ew_t[gv$monthly$q3$measure=="GeoRisk"]),
              cellpt(0, gv$monthly$q3$ew_t[gv$monthly$q3$measure=="GeoRisk"])),
            sprintf("Quarterly (firm) & %s~%s \\\\",
              cellp(gv$quarterly$q3$ew_alpha[gv$quarterly$q3$measure=="GeoRisk"], gv$quarterly$q3$ew_t[gv$quarterly$q3$measure=="GeoRisk"]),
              cellpt(0, gv$quarterly$q3$ew_t[gv$quarterly$q3$measure=="GeoRisk"])))
  writetab("tab_gvkey.tex", tabular("lc", c("Aggregation & GeoRisk EW L/S $\\alpha$~$(t)$\\\\"), rows)) })
...
## Full monthly sort grid (universe x measure x bins x weighting)
local({
...
local({
  ...
  pc <- base + geom_line(linewidth=0.6) + scale_colour_manual(values=c("Q5 (high)"=PASTEL[4],"Q1 (low)"=PASTEL[1]), name=NULL)
...
# =====================================================================
PN <- tryCatch(as.data.table(readRDS(file.path(ana, "panel_ric_v11.rds"))), error=function(e) NULL)
if (is.null(PN)) cat("panel_ric_v11.rds not found -- skipping RQ1 extra figures\n") else {
  ...
  local({
    ...
    g <- rbind(mk("GeoSentimentPos","Positive-tone geo content"),
               mk("GeoSentimentNeg","Negative-tone geo content"))
    ...
    pc <- base + geom_line(aes(colour=leg), linewidth=0.8) + geom_point(aes(colour=leg), size=1.6) +
      scale_colour_manual(values=c("Positive-tone geo content"=PASTEL[3], "Negative-tone geo content"=PASTEL[2]))
  ...
  ## Fig R3 -- RQ1 coefficient forest (contemp return + IVOL), four measures, 95% CI
  local({
    rr <- q1$results; vv <- v1
    df <- rbindlist(lapply(M4, function(m){ a<-rr[rr$measure==m,]; b<-vv[vv$measure==m,]
    ...
    save_both("fig_q1_coef_forest.pdf", pc, pb, h=3.2) })
...
local({
  ...
  PNq <- as.data.table(readRDS(file.path(ana, "panel_ric_v11.rds")))
  ...
  legs <- c(GeoSentiment="GeoSentiment", GeoSentimentPos="Positive tone", GeoSentimentNeg="Negative tone")
  ...
  legs2 <- c(GeoSentiment="GeoSentiment $=$ Pos $-$ Neg", GeoSentimentPos="Positive-tone leg", GeoSentimentNeg="Negative-tone leg")
  ...
  szlab <- c(GeoSentiment="GeoSentiment (net)", Pos="Positive-tone leg")
...
local({
  ...
  ## Fig -- GeoRisk L/S FF5 alpha within size / B-M / momentum terciles (A14)
  clab <- c(size="Size", bm="Book-to-Market", momentum="Momentum"); tlab <- c(T1="Low", T2="Mid", T3="High")
  chr <- rbindlist(lapply(names(clab), function(ch) rbindlist(lapply(names(tlab), function(g){
    x <- as.numeric(unlist(M$characteristics[[ch]]$GeoRisk[[g]])); data.table(char=clab[ch], ter=tlab[g], alpha=x[1], t=x[2]) }))))
  chr[, char:=factor(char, levels=clab)][, ter:=factor(ter, levels=tlab)]
  ...
  save_both("fig_q3_characteristics.pdf",
    ...
    bc + scale_fill_manual(values=c("grey30","grey55","grey78"), name=NULL), h=3.4)
  ...
  r1 <- os$rq1_realization_split$GeoRisk
  inT <- function(meas) { r <- cm$q3[cm$q3$measure == meas, ]; r$ew_t }   # headline in-sample t (Table 4.8)
  rows_oos <- c(
    sprintf("RQ1 realisation (GeoRisk) & split-half & %s & %s \\\\", fnum(r1$H1$t), fnum(r1$H2$t)),
    sprintf("RQ2 tone L/S (GeoSentiment) & re-learned dict.\\ & %s & %s \\\\", fnum(inT("GeoSentiment")), fnum(rtd$GeoSentiment$realtime$t)),
    sprintf("RQ3 GeoRisk L/S (FF5 $\\alpha$) & re-learned dict.\\ & %s & %s \\\\", fnum(inT("GeoRisk")), fnum(rtd$GeoRisk$realtime$t)))
  writetab("tab_oos.tex", tabular("llcc",
    c(" & & \\multicolumn{2}{c}{$t$-statistic}\\\\", "\\cmidrule(lr){3-4}",
      "Research question & Design & In-sample & Out-of-sample\\\\"), rows_oos))
...
## GPR external validation -- quarterly mean GeoExposure vs the Caldara-Iacoviello GPR
## threats index, both z-scored on their own history. The correlation printed on the
## panel is computed from the same file as the GPR_RHO the prose quotes (gpr_quarterly.csv).
local({
  gp <- tryCatch(fread(file.path(root, "out/exposure/gpr_quarterly.csv")), error = function(e) NULL)
  if (is.null(gp)) { cat("gpr_quarterly.csv missing -- fig_gpr_validation skipped\n"); return(invisible()) }
  z <- function(x) (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE)
  d <- data.table(date = gp$year + (gp$quarter - 1) / 4,
                  GeoExposure = z(gp$GeoExposure), GPRthreats = z(gp$GPRT))
  rho <- sprintf("rho == %.2f", cor(gp$GeoExposure, gp$GPRT, use = "complete.obs"))
  dl <- melt(d, id.vars = "date", variable.name = "series", value.name = "v")
  dl[, series := factor(series, levels = c("GeoExposure", "GPRthreats"),
                        labels = c("GeoExposure (mean, z)", "GPR threats (z)"))]
  base <- ggplot(dl, aes(date, v, colour = series, linetype = series)) +
    geom_hline(yintercept = 0, colour = "grey70", linewidth = 0.3) +
    geom_line(linewidth = 0.55) +
    annotate("text", x = min(d$date) + 0.5, y = max(dl$v, na.rm = TRUE),
             label = rho, parse = TRUE, hjust = 0, size = 3.2, family = "serif") +
    labs(x = NULL, y = "z-score") + theme_thesis()
  save_both("fig_gpr_validation.pdf",
    base + scale_colour_manual(values = PASTEL[c(1, 2)]) + scale_linetype_manual(values = c("solid", "solid")),
    base + scale_colour_manual(values = c("black", "grey55")) + scale_linetype_manual(values = c("solid", "22")),
    h = 3.2) })
...
local({
  MM <- fread(file.path(ana, "RESULTS_MASTER_v11.csv"))
  spec <- list(
    ...
    c("Q2_pricing_FM","lambda","US","quarterly","RQ2 Fama-MacBeth (US)"),
    c("Q3_LS_FF5","ew_alpha","CRSP","monthly","RQ3 L/S FF5 alpha (US, EW)"))
  ...
  d[, label := factor(label, levels = rev(sapply(spec, `[`, 5)))]
  ...
  pb <- base + scale_fill_gradient2(low = "grey55", mid = "white", high = "grey55",
                                    limits = c(-6, 6), oob = scales::squish)
  save_both("fig_t_heatmap.pdf", pc, pb, w = 6.8, h = 3.4) })
