...
tc_net <- function(meas) {
  d2 <- p[is.finite(get(meas)) & is.finite(MCap_MEnd) & MCap_MEnd > 0]
  d2[, b := as.integer(ceiling(frank(get(meas), ties.method = "first") / .N * 5)), by = Month]
...
tc_dt[, `:=`(series = factor(series, levels = c("Gross", "Net of costs")), measure = factor(measure, levels = c("GeoRisk", "GeoSentiment")))]
tcw[, measure := factor(measure, levels = c("GeoRisk", "GeoSentiment"))]
# shade the cost drag = the area between the gross and net paths
...
ggsave(file.path(figdir, "fig_q3_netcost.pdf"),
       tcbase + geom_ribbon(data = tcw, aes(Month, ymin = net, ymax = gross), fill = "#F2A6A6", alpha = 0.30) +
         geom_line(data = tc_dt, aes(Month, cum, colour = series), linewidth = 0.75) +
         scale_colour_manual(values = c("Gross" = "#8FB8DE", "Net of costs" = "#D9534F"), name = NULL),
